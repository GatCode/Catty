...
class MultiFingerXFunctionTests: XCTestCase {

    var touchManager: TouchManagerMock!
    var function: MultiFingerXFunction!
    ...
    func testTag() {
        XCTAssertEqual("MULTI_FINGER_X", type(of: function).tag)
    }

    func testName() {
        XCTAssertEqual(kUIFESensorFingerX, type(of: function).name)
    }
