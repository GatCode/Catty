...
-(void)saveImageData:(NSData*)imageData withFileName:(NSString*)imageFileName andImageFileNameExtension:(NSString*)imageFileNameExtension
{
    ...
    NSString *newImagePath = [NSString stringWithFormat:@"%@%@/%@/%@",
                              [self.object projectPath], self.object.scene.name, kProjectImagesDirName, newImageFileName];
...
//        NSDebug(@"Updating");
        NSData *imageData = UIImagePNGRepresentation(image);
        NSString *imageDirPath = [[self.object projectPath] stringByAppendingString:[self.object.scene.name stringByAppendingPathComponent:kProjectImagesDirName]];
